...
export default function Topbar({ header, description, icon: Icon, isSidebarOpen, setIsSidebarOpen }) {
    ...
    return (
        <header className="bg-white/80 backdrop-blur-md min-h-[70px] md:min-h-[80px] shrink-0 flex items-center justify-between px-4 md:px-8 sticky top-0 z-30 border-b border-slate-100/50">
            <div className="flex items-center gap-3 md:gap-6">
                ...
                {Icon && (
                    <div className="w-11 h-11 bg-emerald-50 rounded-xl border border-emerald-100 text-emerald-600 hidden sm:flex items-center justify-center shrink-0">
                        <Icon className="w-5 h-5" />
                    </div>
                )}
                <div className="flex flex-col justify-center gap-1 min-w-0">
                    <h2 className="font-black text-slate-800 text-sm md:text-base tracking-tight leading-none uppercase truncate">{header}</h2>
                    {description && (
                        <p className="text-slate-400 text-[9px] font-bold uppercase tracking-widest leading-none hidden md:block truncate">
                            {description}
                        </p>
                    )}
                </div>
